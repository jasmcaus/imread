...
def _as_gray(im, as_gray):
    if not as_gray or len(im.shape) != 3: return im
    if im.shape[2] == 1:
        return im.squeeze()
...
def imread(filename, as_gray=False, formatstr=None, return_metadata=False, opts=None):
    """
    ...
    filename : str
        filename
    as_gray : boolean, optional
        Whether to convert to gray scale image (default: False)
    ...
    im : ndarray
        ...
        `as_gray`. Conversion from colour to grayscale will return a floating
        point image.
    meta: str
    ...
    reader = special.get(formatstr, cimread.imread)
    imdata,meta = reader(filename, formatstr, opts)
    imdata = _as_gray(imdata, as_gray)
...
def imread_from_blob(blob, formatstr=None, as_gray=False, return_metadata=False, opts=None):
    """
    ...
    **formatstr** is a mandatory parameter to this function!
    ...
    formatstr : str, optional
        ...
        the format.
    as_gray : boolean, optional
        Whether to convert to gray scale image (default: no)
    return_metadata : bool, optional
        Whether to return metadata (default: False)
    opts : dict, optional
        Other options. Support will depend on the backend. Currently used
    ...
    if opts is None:
        opts = {}
    reader = cimread.imread_from_blob
...
def imread_multi(filename, formatstr=None):
    ...
    filename = _norm_path(filename)
    formatstr = _parse_formatstr(filename, formatstr, 'imread')
    return cimread.imread_multi(filename, formatstr, '')
...
def imsave(filename, array, formatstr=None, metadata=None, opts=None):
    ...
    cimread.imsave(filename, formatstr, array, opts)
...
def imsave_multi(filename, arrays, formatstr=None, opts=None):
    ...
    if formatstr is None:
        ...
        formatstr = filename[dot+1:]
    cimread.imsave_multi(filename, formatstr, arrays, opts)
...
def detect_format(filename_or_blob, is_blob=False):
    ...
    return cimread.detect_format(filename_or_blob, bool(is_blob))
...
def supports_format(formatstr):
    ...
    is_supported : bool
    """
    return cimread.supports_format(formatstr)
